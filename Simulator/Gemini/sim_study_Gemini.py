...
RESULTS_FILE = "results_Gemini.csv"
...
#----Added to switch from GPT to Gemini:
#---- API function helper
client = genai.Client()
    ...
    for attempt in range(1, 4):
        try:
            ...
            if not text:
                ...
                raise RuntimeError(f"empty text, finish_reason={finish}")
            return text
...
def main():
    ...
    for persona in personas.itertuples():
        for idx, condition in item_plan(persona, design):
            ...
            reply = call_gemini(system=persona_prompt(persona), user=item_prompt(item, condition))
            time.sleep(SLEEP)
