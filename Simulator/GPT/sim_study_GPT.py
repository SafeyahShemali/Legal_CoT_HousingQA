...
def call_api(system, user, temperature=TEMPERATURE):
...
def main():
    ...
    for persona in personas.itertuples():
        for idx, condition in item_plan(persona, design):
            ...
            reply = call_api(system=persona_prompt(persona), user=item_prompt(item, condition))
            answer_raw, usefulness_raw = parse_json(reply)

            # .get gives None for a FAILED call or an unrecognised label, so answer_binary is
            # always defined: an undefined one would either crash the row build or silently
            # carry the previous item's value into this row.
